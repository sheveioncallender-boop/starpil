...
- Confirm direct native Odoo routes (`/shop/cart`, `/shop/checkout`, `/shop/address`, `/shop/payment`) redirect approved users to the matching Starpil page and unapproved visitors to the Pro Account flow.
...
- Place an order through the single Starpil checkout and confirm its address, carrier, payment transaction, quotation/sales order, invoice and stock workflow use native Odoo records.
- Confirm no second Odoo checkout page appears before or after payment.
- Confirm the custom customer dashboard, order, invoice, profile and rewards pages read the current Odoo records.
