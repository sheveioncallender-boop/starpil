...
| `starpil_website` | Approved V11 white/deep-pink storefront, live catalogue, protected prices, custom cart, one-page checkout and customer dashboard—all powered by Odoo records |
...
- Product, cart and the single Starpil checkout show estimated points from the current customer's tier, eligible spend, product multiplier and quantity.
- Odoo's native storefront/cart/checkout pages cannot create a second customer experience; visible traffic is returned to the matching Starpil page while Odoo continues to process the order, delivery method and payment transaction.
...
1. Administrators have full access. Assign the **Pro Application Manager** and **Loyalty Manager** groups only to non-administrator staff who should manage those areas.
...
The approved HTML mentions WiPay, but a live WiPay payment provider is not included in this release because its API mode, credentials, webhook signature rules and production URLs must be confirmed from the merchant account. The customer sees only the custom Starpil checkout; Odoo's installed payment providers remain the transaction engine embedded inside it. Add `starpil_wipay` only after those details are available.
...
This V3 package brings the approved public, Pro-account, cart, one-page checkout, confirmation, customer dashboard, orders, invoices, profile and loyalty experiences into live Odoo QWeb pages. Odoo remains the engine for products, inventory, prices, orders, delivery, providers, transactions, invoices and rewards. Payment-provider modules and the streamlined shipping/email administration screens remain later implementation milestones; their approved HTML screens are preserved in `design-preview`.
